...
public class XssRequestWrapper extends HttpServletRequestWrapper {
  ...
  private String stripXSS(String value) {
    ...
    try {
      cleanedValue = ESAPI.encoder().canonicalize(cleanedValue);
    } catch (Exception e) {
      cleanedValue = "";
    ...
    cleanedValue = cleanedValue.replaceAll("\0", "");

    // Remove all sections that match a pattern
    for (Pattern scriptPattern : patterns) {
      cleanedValue = scriptPattern.matcher(cleanedValue).replaceAll("");
    }

    // With info on this is a *valid* log injection attack
    if (!cleanedValue.equals(value)) {
      log.info("Cleaned [%] to [%]", value, cleanedValue);
    }
    return cleanedValue;
  }
}
