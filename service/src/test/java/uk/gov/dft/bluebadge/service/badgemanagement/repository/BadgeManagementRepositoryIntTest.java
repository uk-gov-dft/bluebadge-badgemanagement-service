...
public class BadgeManagementRepositoryIntTest extends ApplicationContextTests {
    ...
    @Autowired
    BadgeManagementRepository badgeManagementRepository;
    ...
    public void findBadgesForPrintBatch_shouldSearchByBatchTypeStandard() {
        BadgeEntity expectedBadgeEntity = BadgeEntity.builder().badgeNo("KKKKKA").badgeStatus(Status.ORDERED).partyCode("PERSON").localAuthorityShortCode("ABERD").localAuthorityRef("to update").appDate(LocalDate.of(2018, 6, 1))
                .appChannelCode("ONLINE").startDate(LocalDate.of(2025, 5, 1)).expiryDate(LocalDate.of(2028, 5, 1)).eligibilityCode("PIP").imageLink("").imageLinkOriginal(null)
                .deliverToCode("HOME").deliverOptionCode("STAND").holderName("Reginald Pai").nino("").dob(LocalDate.of(1953, 9, 12)).genderCode("MALE").contactName("contact name").contactBuildingStreet("building and street")
                .contactLine2("").contactTownCity("Town or city").contactPostcode("S637EU").primaryPhoneNo("020 7014 0800").secondaryPhoneNo(null).contactEmailAddress("test101@mailinator.com").cancelReasonCode(null).replaceReasonCode(null).orderDate(null)
                .numberOfBadges(0).build();
